...
% href stuff
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\urllink}[2]{\href{#1}{\textcolor{SlateGray}{\uline{#2}}}}
...
\title[ROS 2 -- Part I]{\cnordZero{ROS 2 Training} -- \cnordTwo{Part I}}
\subtitle{Environment setup and basic notions}
\newcommand{\titleAuthor}{Lecturer}
\author{Z. Kootbally}
% \newcommand{\titleInstitute}{Institute}
% \institute{NIST}
% \newcommand{\titleMiscI}{Course}
% \newcommand{\descMiscI}{[NIST] ROS2 Nist Training}
% \newcommand{\titleMiscII}{File}
% \newcommand{\descMiscII}{\currfilebase}
\date{\today}
\titlegraphic{NIST_logo.png}
...
{ \myDefinitionIcon Syllabus} \doublerulefill

\begin{compactitem}
\footnotesize
    \item Part I -- Environment setup, basic notions, and workspaces.
    \item Part II -- Writing/Executing nodes.
    \item Part III -- Transforms.
    \item Part IV -- Custom interfaces.
    \item Part V -- ROS Bags and visualization.
...
\begin{center} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{\mydefinition Shell} \doublerulefill
...
\begin{center} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{\mydefinition Terminal} \doublerulefill
...
{\mydefinition bashrc File} \doublerulefill
...
{\mycodesyntax Functions} \doublerulefill
...
{\mycodesyntax Function Selection} \doublerulefill
...
{\mydefinition Aliases} \doublerulefill
...
{\mynote Bash Command: \textbf{source}} \doublerulefill
...
{\mydefinition ROS} \doublerulefill
...
{\mynote Resources} \doublerulefill
...
{\mynote ROS1: Drawbacks} \doublerulefill
...
\vspace*{\fill}
\begin{center} 
{\mynote ROS1: Missing Use Cases} \doublerulefill
...
\vspace*{\fill}
\begin{center} 
{\mynote ROS2: Overview} \doublerulefill
...
{\mynote DDS: Data Distributed Service} \doublerulefill
...
\vspace*{\fill}
\begin{center} 
{\mytodo Check DDS Version} \doublerulefill
...
{\mytodo Install Extensions for VS Code} \doublerulefill
    ...
    \begin{compactitem}
        ...
        \item \urllink{ext install donjayamanne.python-extension-pack}{Python Extension Pack}
    \end{compactitem}
...
{\mytodo Install \textbf{colcon}} \doublerulefill
...
{\mydefinition Node} \doublerulefill

\emph{A \urllink{http://wiki.ros.org/ROS/Tutorials/UnderstandingNodes}{Node} is a small program written in Python or \CC that executes some relatively simple task or process. Nodes are usually categorized into Publishers (send data) and Subscribers (receive data).} 


{\mydefinition Topic} \doublerulefill

\emph{A \urllink{http://wiki.ros.org/Topics}{Topic} is a named bus\footnote{A communication system that transfers data between components inside a computer or between computers} over which Nodes exchange Messages.}

{\mydefinition Message} \doublerulefill

\emph{A \urllink{http://wiki.ros.org/Messages}{Message} is a packet of data (a simple data structure comprising typed fields).}
...
{\mynote Running Nodes} \doublerulefill
...
{\mytodo Run only a single Node} \doublerulefill
...
\mynote An executable in \CC is the machine code of your program (after compiling and linking the source code). An executable in Python is a Python script (usually a \myFile{*.py} file) or an executable generated from \myFile{setup.py}.
...
{\mytodo Run a launch file} \doublerulefill
...
{\myinfo Introspect Nodes and Topics} \doublerulefill
...
\vspace*{\fill}
\begin{center} 
{\mydefinition Workspace} \doublerulefill

\emph{A ROS workspace is a folder where you write all packages  needed to build a ROS2 application.}

{\mytodo Create a Workspace} \doublerulefill

\begin{compactitem}
\item Create a workspace anywhere you want and give it a name of your choice.
\item[]\myTerminal{mkdir -p \textasciitilde/nist_ws/src}
\begin{compactitem}
...
\end{compactitem}
\item Build the workspace: \myTerminal{cd \textasciitilde/nist_ws} followed with \myTerminal{colcon build}
\begin{compactitem}
\item \mywarning You have to be in the root folder of the workspace to build it.
\item \mynote The build process looks at packages inside \myFolder{src}  and build them. As of now, there is no package in \myFolder{src} but some folders are still generated.
\end{compactitem}
\end{compactitem}
\end{center}
\vspace*{\fill}
...
\vspace*{\fill}
\begin{center} 
{\mydefinition Package} \doublerulefill
...
\vspace*{\fill}
\begin{center} 
{\mytodo Create the \CC package \myROSPackage{first_package_cpp}}
...
\begin{compactitem}
\footnotesize
\item \myTerminal{cd \textasciitilde/nist_ws/src}
...
{\mytodo Create the Python package \mypackageNormal{first_package_py}}

\myreminder Packages are created in \mydirNormal{\textasciitilde/nist_ws/src}
...
{\mynote Python Packages} \doublerulefill
...
{\mytodo Create a package for both \CC and  Python:~ \myROSPackage{first_package}} 
...
\begin{forest}
  ...
  for tree={% folder icons by default; override using file for file icons
    ...
    [
        \small~first_package
        [
            \small~first_package \textcolor{BrickRed}{$\rightarrow$ Python interfaces}
            [
                \small __init__.py, file
            ]
        ]
    [\small~nodes\textcolor{BrickRed}{$\rightarrow$ Python nodes}]
    [\small~include\textcolor{BrickRed}{$\rightarrow$ C++ interfaces (.hpp files)}]
    [\small~src\textcolor{BrickRed}{$\rightarrow$ C++ implementation (.cpp files)}]
    [\small CMakeLists.txt, file]
    [\small package.xml, file]
  ]
\end{forest}
...
{\large \myinfo \cnordZero{Part II:} Nodes -- Publishers and Subscribers} \doublerulefill
